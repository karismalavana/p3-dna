...
public interface IDnaStrand extends Iterable<Character>{
	...
	default IDnaStrand cutAndSplice(String enzyme, String splicee) {
		...
		//while loop runs b times --> overall big O: bN
		while ((pos = search.indexOf(enzyme, start)) >= 0) { //O(N)
			if (first) {
				ret = getInstance(search.substring(start, pos));
				first = false;
			} else {
				ret.append(search.substring(start, pos)); //O(1)

			}
			start = pos + enzyme.length();
			ret.append(splicee);//O(1)
		}
		if (start < search.length()) {
			// NOTE: This is an important specialf case! If the enzyme
			// is never found, return an empty String.
			if (ret == null) {
				ret = getInstance("");
			} else {
				ret.append(search.substring(start)); //O(1)
			}
		}
		return ret;
	}
